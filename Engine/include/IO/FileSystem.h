...
namespace Luden 
{
	...
	enum class ENGINE_API FileStatus
	{
	...
	public:
		...
		static bool DeletePath(const std::filesystem::path& filepath);
		static bool MovePath(const std::filesystem::path& filepath, const std::filesystem::path& dest);
	...
	public:
		...
		static bool LudenSetEnvironmentVariable(const std::string& key, const std::string& value);
		static std::string LudenGetEnvironmentVariable(const std::string& key);
	};
}