...
namespace Luden {
	...
	Entity Scene::CreatePrefabEntity(Entity entity, Entity parent, const glm::vec3* translation, const glm::vec3* rotation, const glm::vec3* scale)
	{
		...
		if (rotation) newTransform.angle = rotation->x;
		else newTransform.angle = entity.Get<TransformComponent>().angle;
		...
		if (m_IsPlaying)
		{
			if (newEntity.Has<NativeScriptComponent>()) {
				newEntity.Get<NativeScriptComponent>().CreateInstance(newEntity);
			}
